CREATE OR REPLACE FUNCTION calculate_and_store_iaa(citation_id_in INTEGER)
RETURNS VOID AS $$
DECLARE
    all_locations INT[][];
    loc_count INTEGER;
...
BEGIN
    ...
    WHERE citation_id = citation_id_in
      AND (annotation_location @> ARRAY[1] OR annotation_location @> ARRAY[2] OR annotation_location @> ARRAY[3]);
    ...
    FOR i IN 1..loc_count LOOP
        FOR j IN (i + 1)..loc_count LOOP
            actual_array := all_locations[i];
            predicted_array := all_locations[j];
    ...
    IF pair_count > 0 THEN
        ...
        SET 
            iaa_macro = total_f1_macro / pair_count,
            iaa_total = total_f1_total / pair_count,
            iaa_inf = total_f1_cls[1] / pair_count,
            iaa_perc = total_f1_cls[2] / pair_count,
            iaa_back = total_f1_cls[3] / pair_count
